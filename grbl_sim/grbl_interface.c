...
void print_steps(bool force)
{
  ...
  if (SPINDLE_TCCRA_REGISTER >= 127)
    ocr = SPINDLE_OCR_REGISTER;
#endif
  ...
  if (current_block != printed_block)
  {
    //new block.
    if (block_number)
    { //print values from the end of prev block
      fprintf(args.step_out_file, "%12.5f %d, %d, %d, %d, %d, %d\n", sim.sim_time, sys_position[X_AXIS], sys_position[Y_AXIS], sys_position[Z_AXIS], sys_position[A_AXIS], sys_position[B_AXIS], ocr);
    }
...
{
  if (get_block_buffer_head() == get_block_buffer_tail())
  {
    return (NULL);
  }
...
void printBlock()
{
  ...
  if (b != last_block && b != NULL)
  {
    ...
    for (i = 0; i < N_AXIS; i++)
    {
      if (b->direction_bits & get_direction_pin_mask(i))
